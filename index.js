  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const message of messages) {
      try {
        ...
        // ה-JID הגולמי נרשם תמיד. בלעדיו אי אפשר להבחין בין הודעה פרטית
        // לבין הודעה בקבוצה — כי בקבוצה `participantPn` הוא המספר של השולח,
        // ושתיהן נראות זהות בלוג. ההבחנה הזו עלתה לנו סיבוב שלם של אבחון.
        const ctx = `${from} [${jid}${message?.key?.fromMe ? " fromMe" : ""}]`;

        // ── סינון לפי סוג הצ'אט — לפני כל דבר אחר ──
        //
        // הסדר קריטי: כישלון פענוח בקבוצה הוא שכיח ולא מזיק (אין עדיין
        // sender-key לקבוצות קיימות אחרי חיבור מכשיר חדש). אם בודקים אותו
        // לפני הסינון, כל רעש קבוצתי נראה כמו הזמנה פרטית שאבדה.
        if (jid.endsWith("@g.us")) {
          log.info(`  ⤷ ${ctx}: מדולג — קבוצה`);
          continue;
        }
        if (jid.endsWith("@newsletter") || jid === "status@broadcast") {
          log.info(`  ⤷ ${ctx}: מדולג — ערוץ/סטטוס`);
          continue;
        }
        if (message?.key?.fromMe) {
          log.info(`  ⤷ ${ctx}: מדולג — נשלחה מהמספר המחובר עצמו`);
          continue;
        }

        // ── הודעה פרטית שלא פוענחה — זה כבר רציני ──
        //
        // ‏"No session found to decrypt message" — אין סשן Signal מול השולח.
        // ‏Baileys מגיבה אוטומטית: שולחת receipt מסוג retry, ואחרי 5 שניות
        // מבקשת מהטלפון שסרק לשדר את התוכן מחדש. אם זה מצליח, ההודעה תגיע
        // שוב כאירוע נפרד — ואז היא תיקלט כרגיל.
        if (message?.messageStubType === CIPHERTEXT_STUB) {
          log.warn(
            `  ⤷ ${ctx}: הודעה פרטית לא פוענחה. Baileys ביקשה שידור חוזר — ` +
              `אם לא מגיעה שורת "מעביר לבקאנד" תוך ~10 שניות, השידור החוזר נכשל.`
          );
          continue;
        }

        if (!message?.message) {
          log.info(`  ⤷ ${ctx}: מדולג — בלי תוכן (stub=${message?.messageStubType ?? "—"})`);
          continue;
        }

        const payload = await collectMessage(message, { sock, logger: log });
        if (!payload) {
          log.info(`  ⤷ ${ctx}: מדולג — אין טקסט ואין קובץ (תגובה/אישור קריאה?)`);
          continue;
        }
